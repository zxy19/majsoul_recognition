...
class Net(torch.nn.Module):
    def __makeLayerBlock(self,block,batchNormBlock,inputChannel,activation,*arg):
        tmpRet = torch.nn.Sequential()
        tmpRet.append(block(*arg))
        tmpRet.append(batchNormBlock(inputChannel))
    ...
    def __init__(self):
        ...
        #80 * 80 * 12
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,36,torch.nn.LeakyReLU,12,24,36))
        #80 * 80 * 36
        self.layer2d.append(torch.nn.MaxPool2d(2))
        #40 * 40 * 36
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,60,torch.nn.LeakyReLU,36,48,60))
        #40 * 40 * 60
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,84,torch.nn.LeakyReLU,60,72,84))
        #40 * 40 * 84
        self.layer2d.append(torch.nn.MaxPool2d(2))
        #20 * 20 * 84
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,108,torch.nn.LeakyReLU,84,96,108))
        #20 * 20 * 108
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,132,torch.nn.LeakyReLU,108,120,132))
        #20 * 20 * 132
        self.layer2d.append(torch.nn.MaxPool2d(2))
        #10 * 10 * 132
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,156,torch.nn.LeakyReLU,132,144,156))
        #10 * 10 * 156
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,180,torch.nn.LeakyReLU,156,168,180))
        #10 * 10 * 180
        self.layer2d.append(torch.nn.MaxPool2d(2))
        #5 * 5 * 180
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,204,torch.nn.LeakyReLU,180,192,204))
        #5 * 5 * 204
        self.layer2d.append(self.__makeLayerBlock(ResidualBlock,torch.nn.BatchNorm2d,228,torch.nn.LeakyReLU,204,216,228))
        #5 * 5 * 228
        self.layer1d.append(self.__makeLayerBlock(torch.nn.Linear,torch.nn.BatchNorm1d,1000,torch.nn.LeakyReLU,5*5*228,1000))
    ...
    def forward(self, x):
        ...
        for layer in self.layer2d:
            out = layer(out)
            #out = torch.nn.functional.dropout2d(out,0.5)
        out = out.view(out.size(0),-1)
        for layer in self.layer1d:
            out = layer(out)
            #out = torch.nn.functional.dropout(out,0.5)
        return out
model = Net()
if(use_gpu):
    model.cuda()
